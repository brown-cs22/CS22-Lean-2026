...
What properties should the `gcd` algorithm have?
...
theorem gcd_divides : ∀ a b : ℕ, gcd a b ∣ a ∧ gcd a b ∣ b := by
  ...
  { simp [hba]
    ...
    { simp [ha]
      ...
      -- It's a property of mod that `b % a < a`.
      have h_bmoda : b % a < a := Nat.mod_lt _ (by positivity)

      -- We can use this again instantiate our induction hypothesis.
      ...
      { dsimp [Dvd.dvd] at hl hr ⊢
        ...
        have hqr : a * (b / a) + b % a = b := Nat.div_add_mod b a
